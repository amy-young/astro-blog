...
description: A system I built to keep up with AI has turned into something that works less like software and more like a colleague. Here's the architecture, and what it takes to build one yourself.
...
<style>
  /* Rewritten 2026-08-17 off Tailwind utility classes onto plain CSS -- this diagram is raw
     HTML embedded in markdown, so it sits outside home.css entirely and was styled purely
     by Tailwind utilities (dark:, rounded-xl, text-[10px], etc). Removing Tailwind sitewide
     (main.astro was its last consumer) left every class here resolving to nothing: the
     onclick interactivity kept working, but the whole diagram rendered as unstyled markup.
     No dark: variants reintroduced -- the site is light-only by design (post.astro's own
     2026-08-14 ruling), so this diagram now matches that instead of carrying dead classes. */
